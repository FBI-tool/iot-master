...
class Project extends EventEmitter {
    ...
    constructor(model) {
        ...
        //将变量取出来
        //this.context.set(model.variables);
        Object.assign(this.context, model.parameters);
    ...
    init(model) {
        ...
        model.variables && model.variables.forEach(v => {
            //this.addVariable(v);
            if (this.context.hasOwnProperty(v.name)) return;
            this.context[v.name] = v.default && eval(v.default); //计算默认值
        })
        ...
        model.strategies && model.strategies.forEach(s => {
            if (!s.enable) return;
            this.addStrategy(s);
        })
    ...
    addStrategy(j) {
        const s = new Strategy(j, this.context);
        this.strategies.push(s);

        s.on('error', err => this.emit('error', err));
        s.on('execute', (command, params) => {
            this.execute(command, params);
            this.createEvent('控制策略：' + command);
        })
    }
